...
GoogleParseAuth.prototype.makeAuthenticatedCall = function(/* userId, object, method, ... arguments ..., callback*/){
	console.log("Making authenticated call");
	...
	function callbackTrap(err, data) {
		...
		if(err && err.error.code == 401) {
			console.log("Not logged in error.");
			callback(err, null);
		} else if((!err || !(err instanceof Error)) && !err.code) {
			console.log("Success! Passing through");
			if (err) {
				console.log("...but theres an error: ", err);
			}
			callback(err, data);
		} else {
			console.log("Got a bad token, trying again");
			T.refreshAccessToken(userId, function(accessToken) {
				console.log("Refreshed, got access token: ", accessToken);
